...
import { Bookmark, Users, User, Trash2 } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "./ui/card";
import type { Recipe, HouseholdMember } from "@/lib/types";
...
const RecipeCard = ({ recipe, onRemoveClick, isOwner }: { recipe: Recipe, onRemoveClick?: (recipe: Recipe) => void, isOwner: boolean }) => (
...
export function SavedRecipes({ initialRecipes, initialHouseholdRecipes }: { 
  ...
  useEffect(() => {
    const backfillCalories = async (recipes: Recipe[], setter: React.Dispatch<React.SetStateAction<Recipe[]>>) => {
        ...
        if (recipe.macros && typeof recipe.macros.calories === 'undefined') {
          ...
          try {
            ...
            if ('servings' in result) {
              ...
              setter(prev => prev.map(r => r.title === updatedRecipe.title ? updatedRecipe : r));
              toast({
    ...
    backfillCalories(myRecipes, setMyRecipes);
    backfillCalories(householdRecipes, setHouseholdRecipes);
  ...
  const handleConfirmRemove = async () => {
    ...
    if (result.success) {
        toast({ title: "Recipe Removed", description: `"${recipeToDelete.title}" has been removed from your saved recipes.`});
      ...
      return (
        <Accordion type="single" collapsible className="w-full space-y-4">
            {recipes.map((recipe, index) => (
                ...
                <Card key={`${recipe.title}-${index}-loading`} className="p-4">
                ...
                :
                <RecipeCard key={`${recipe.title}-${index}`} recipe={recipe} onRemoveClick={isOwner ? handleRemoveClick : undefined} isOwner={isOwner} />
            ))}
        </Accordion>
      )
  };
