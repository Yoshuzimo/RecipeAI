...
import { Loader2, PlusCircle, Trash2, UtensilsCrossed } from "lucide-react";
...
export function LogManualMealDialog({ onMealLogged }: { onMealLogged: () => void }) {
    ...
    const onSubmit = async (data: FormData) => {
        ...
        const result = await handleLogManualMeal(foodArray, data.mealType);
        ...
        if (result.success) {
            ...
            onMealLogged();
            setIsOpen(false);
            form.reset({ foods: [{ value: "" }], mealType: getDefaultMealType() });
        } else {
            toast({
                variant: "destructive",
